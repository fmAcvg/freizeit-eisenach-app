...
export default function LocationPicker({
  ...
  useEffect(() => {
    console.log('🔄 LocationPicker value changed:', value);
    setInputValue(value);
  }, [value]);
  ...
  const handlePredictionSelect = (prediction: PlacePrediction) => {
    ...
    console.log('🎯 Main text:', prediction.main_text);
    console.log('🎯 Secondary text:', prediction.secondary_text);
    
    // Die KOMPLETTE Adresse einsetzen (description, nicht nur main_text)
    const fullAddress = prediction.description;
    console.log('🎯 Setting full address:', fullAddress);
    
    // SOFORTIGE Aktualisierung - keine Verzögerungen
    ...
    // Debounce-Timeout löschen
    if (debounceRef.current) {
      clearTimeout(debounceRef.current);
      debounceRef.current = null;
    }
    
    console.log('✅ Prediction selection completed');
  };
  ...
  const renderPrediction = ({ item }: { item: PlacePrediction }) => {
    const handlePress = () => {
      ...
      console.log('🔥 Current input value before:', inputValue);
      
      // SOFORT die komplette Adresse einsetzen
      ...
      console.log('🔥 Input value after:', fullAddress);
    ...
    return (
      <TouchableOpacity
        ...
        onPress={handlePress}
        onPressIn={() => {
          console.log('🔥 Press started on:', item.main_text);
        }}
        activeOpacity={0.5}
        delayPressIn={0}
        delayPressOut={0}
    ...
    <View style={[styles.container, style]}>
      ...
      <View style={[styles.inputContainer, { backgroundColor: surfaceColor }]}>
        ...
        <TextInput
          ...
          onBlur={() => {
            // Kein automatisches Schließen - nur über Touch-Events
          }}
        />
